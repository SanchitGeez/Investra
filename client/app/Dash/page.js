...
const Dash = () => {
  ...
  const { username, balance, userId } = JSON.parse(
    getCookieValue("activeUser")
  ) || { username: "error", balance: "error", userId: "error" };
  ...
  const getBalance = async () => {
    ...
    const b = await axios.post(
      "https://investra-26xe.vercel.app/getBalance",
      req
    );
  ...
  const handleBalanceAdd = async () => {
    ...
    const res = await axios.post(
      "https://investra-26xe.vercel.app/addBalance",
      req
    );
  ...
  const handlePurchase = async () => {
    try {
      ...
      const res = await axios.post(
        "https://investra-26xe.vercel.app/stocks/purchase",
        PurchaseData
      );
      notify(res.data);
  ...
  const updateStocks = async () => {
    try {
      ...
      const res = await axios.post(
        "https://investra-26xe.vercel.app/stocks/update",
        {}
      );
      notify(res.data);
      getStocks();
  ...
  const getStocks = async () => {
    try {
      ...
      const res = await axios.post(
        "https://investra-26xe.vercel.app/stocks/get",
        {}
      );
      //console.log(res);
  ...
  const handleLogout = (e) => {
    ...
    document.cookie =
      "activeUser=null; expires=Thu, 01 Jan 1970 00:00:00 GMT; path=/";
    router.push("/");
  };
  const calculateBasicInfo = async () => {
    ...
    const roundedInvested = Math.ceil(totalInvested * 100.0) / 100.0;
    ...
    <>
      ...
      <div className="container">
        <div className="topbar">
          <p className="text-17xl font-extrabold" style={{ color: "#fea240" }}>
            Hey, {Username}
          </p>
          <div className="top-buttons">
            <button
              className="refresh-button letter-space cursor-pointer"
            ...
            <Popover.Root>
              <Popover.Trigger asChild>
                <button className="balance-button text-black letter-space cursor-pointer">
                  + BALANCE
                </button>
              </Popover.Trigger>
              <Popover.Portal>
                <Popover.Content className="PopoverContent" sideOffset={5}>
                  ...
                  >
                    ...
                    <fieldset className="Fieldset">
                      ...
                      />
                      <button
                        className="balance-add-button cursor-pointer"
            ...
            <button
              className="logout-button letter-space cursor-pointer"
          ...
          <div className="left-side">
            <div className="overall-stats">
              ...
              <div className="pl">
                ...
                <p className="mx-8 bg-lime-600 px-4 rounded-2xl font-semibold">
                  {Invested != 0
                    ? parseFloat(
                        (((Current - Invested) * 100) / Invested).toFixed(2)
                      )
                    : "--"}
                  %
                </p>
            ...
            <div className="balance-buy">
              ...
              <div className="buy-stocks">
                ...
                <button
                  className="buy-confirm cursor-pointer"
                  ...
                  {UserStocks.map((stock) => {
                    ...
                    return (
                      <StockCard
                        ...
                        ticker={stock.ticker}
                        inv={
                          Math.ceil(stock.price * stock.quantity * 100.0) /
                          100.0
                        }
                        qty={stock.quantity}
                        avg={Math.ceil(stock.price * 100.0) / 100.0}
                        ltp={stock.ltp}
                        net={
                          Math.ceil(stock.ltp * stock.quantity * 100.0) / 100.0
                        }
                        pl={parseFloat(
                          (
                            stock.ltp * stock.quantity -
